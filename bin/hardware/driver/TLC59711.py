...
import hardware.driver.settings as drSet
# Driver to send outputs to TLC59711's.
# Also, note: Currently written to handle only one SPI channel.
...
# Each of the functions below are expected by the web interface/system.

def choices():
    # Return 2-tuples for Django for options
    chanChoice = (
        (26, 'R0'),
        ('24', 'G0'),
        ('22', 'B0'),
...
def name():
    # Return a string with the text that the web interface should use.
    return 'TLC59711 PWM'

def hwType():
    # Return the index of the hardware type that this driver consumes.  See
    # hardware.models.hwChoices for the list.
    return 3

def devType():
    # Return 0 for sensor and 1 for control.
    return 1

def start(recID, busID):
    # Do initialization actions for this driver.

    # Test to see if the cape is already loaded, if not, load it.
    f = open(drSet.CAPE_DIR, "rb")
    capes = f.read()
    f.close()
    ...
    if not name in capes:
        f = open(drSet.CAPE_DIR, "wb")
        f.write(name)
        f.close()
